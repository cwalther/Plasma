...
void    plFont::IRenderChar8To32AlphaPremShadow( const plFont::plCharacter &c )
{
    ...
    //static uint32_t div = 80; //FIXME make these const, they aren't yet so I can change them at runtime
    //static uint32_t clamp = 96;
    static uint32_t kernel[5][5] = {
        {1,  2,  2,  2, 1},
        {1, 13, 13, 13, 1},
        {1, 10, 10, 10, 1},
        {1,  7,  7,  7, 1},
    ...
    //clamp = 255 + 96 - (srcR + srcG + srcB)/3;
    //clamp = 287 - (srcR + srcG + srcB)/4;
    //clamp = 236 - (srcR + srcG + srcB)/6;
    uint32_t clamp = 220 - ((2*srcR + 4*srcG + srcB) >> 4);
    //div = 96*80/clamp;

    for( y = -2; y < (int32_t)c.fHeight+2; y++ )
    {
        destPtr = destBasePtr;
        for( x = -2; x < (int32_t)thisWidth+2; x++ )
        {
            uint32_t sa = 0;
            ...
            //FIXME how to ensure we're not overrunning here?
            if (ta > (destPtr[ x ] >> 24))
                destPtr[ x ] = ( ta << 24 ) | (((srcR*a + 127)/255) << 16) | (((srcG*a + 127)/255) << 8) | ((srcB*a + 127)/255);
        }
        destBasePtr = (uint32_t *)( (uint8_t *)destBasePtr + fRenderInfo.fDestStride );
        src += fWidth;
    }
